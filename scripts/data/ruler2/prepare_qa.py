...
def generate_samples(num_samples: int, max_seq_length: int, incremental: int = 5):
    ...
    if max_seq_length > 0:
        ...
        lower_bound = min_docs
        upper_bound = max(estimated_max_docs, max(incremental, min_docs) * 2)  # Ensure upper_bound is reasonable
        ...
        num_docs = optimal_num_docs if optimal_num_docs is not None else min_docs
    ...
    for index in tqdm(range(num_samples)):
        ...
        while True:
            try:
                ...
                if used_docs > min_docs:
                    used_docs -= 1
                else:
                    raise AssertionError(
                        f"Unable to fit sample within max_seq_length={max_seq_length} even at minimum "
                        f"num_docs={min_docs}. Consider using a larger max_seq_length."
                    )

...
def generate_samples(num_samples: int, max_seq_length: int, incremental: int = 500):
    ...
    lower_bound = min_haystack
    upper_bound = max(estimated_max_questions, max(incremental, min_haystack) * 2)  # Ensure upper_bound is reasonable
    ...
    num_haystack = optimal_num_haystack if optimal_num_haystack is not None else min_haystack
    ...
    for index in tqdm(range(num_samples)):
        ...
        while True:
            try:
                ...
                if used_haystack > min_haystack:
                    used_haystack -= 1
                else:
                    raise AssertionError(
                        f"Unable to fit sample within max_seq_length={max_seq_length} even at minimum "
                        f"num_haystack={min_haystack}. Consider using a larger max_seq_length."
                    )
